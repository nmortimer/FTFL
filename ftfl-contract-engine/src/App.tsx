    ...
    <table>
      ...
      <tbody>
        {rows.map(({ contract, salary, yearsLeft, penalty }) => {
          ...
          const anchor = finalYearSalary(contract);
          return (
            <tr key={contract.id}>
              <td>
                {contract.playerName}
              ...
              <td>
                <div className="contract-cell">
                  ...
                  <span className="contract-resign">
                    Resign: {money(anchor)} ({resignYear})
                  </span>
                </div>
              </td>
  ...
  return (
    <>
      <div className="hero-slot">
        <span>Team stadium + jersey art — coming soon</span>
      </div>
      ...
      <p className="footnote">
        "Resigns" shows the year the current deal ends + 1, with the final contracted year's salary as a
        reference point — not a market-value forecast. Real re-sign cost is whatever the player's market value
        is at that time, which isn't in this dataset yet. Cut penalty: 50% of each remaining year's salary,
        rounded up.
      </p>
    </>
  );
